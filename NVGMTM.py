...
parser.add_argument('--gamma', type=float, default=0.001, metavar='N',
    help="beta")
parser.add_argument('--model-save-path', default='save_model/', metavar='N',
    help="Directory for corpus")
...
print('MODEL_SAV_PATH: '+MODEL_SAV_PATH)
#====================================================================================================================================================
...
class Generative_model(nn.Module):
    def __init__(self, vocabulary_size=2000, n_topics=50, topic_embeddings_size=128):
        ...
        #!!! self.mu_l=nn.Linear(vocabulary_size,n_topics)
        # self.log_sigma2_l=nn.Linear(vocabulary_size,n_topics)
        ...
        word_embeddings_mat = Parameter(torch.Tensor(vocabulary_size, topic_embeddings_size))
        torch.nn.init.xavier_uniform_(word_embeddings_mat.data, gain=1)
    ...
    def log_pdf(self, mu, logsig,vec):
        ...
        for k in range(self.n_topics):
            pdf=-0.5*torch.sum(np.log(np.pi*2)+logsig[k:k+1,:]+(vec-mu[k:k+1,:]).pow(2)/torch.exp(logsig[k:k+1,:]),1).view(1,-1)
            # print(pdf.size())
            G.append(pdf)
    ...
    def forward(self, mu, logsig, doc_bow):
        ...
        # print(type(self.word_embeddings_mat.t))

        # word_T = torch.from_numpy(self.word_embeddings_mat.t.data.numpy())
        # self.mu_l(word_T)
        # z_mu=self.mu_l(word_T).t
        # z_sigma2_log=self.log_sigma2_l(word_T).t
        
        # word_matrix = self.word_embeddings_mat.repeat(self.n_topics,1,1)
        # mu_matrix = z_mu.repeat(self.vocabulary_size,1,1).permute(1,0,2)
        # z_sigma2_log_matrix = z_sigma2_log.repeat(self.vocabulary_size,1,1).permute(1,0,2)



        # word_matrix = self.word_embeddings_mat.repeat(self.n_topics,1,1)
        # mu_matrix = self.mu_mat.repeat(self.vocabulary_size,1,1).permute(1,0,2)
        # z_sigma2_log_matrix = self.log_sigma2_mat.repeat(self.vocabulary_size,1,1).permute(1,0,2)


        # print('word_matrix',word_matrix.size())
        # print('mu_matrix',mu_matrix.size())
        # print('z_sigma2_log_matrix',z_sigma2_log_matrix.size())


        # z_mu, z_sigma2_log = self.gmmencoder(self.word_embeddings_mat)
        # k_z = torch.randn_like(z_mu) * torch.exp(z_sigma2_log / 2) + z_mu
        # if hasattr(torch.cuda, 'empty_cache'):
        #     torch.cuda.empty_cache()


        # self.gaussian_pdf = 0.5*torch.sum(np.log(np.pi*2)+z_sigma2_log_matrix+(word_matrix-mu_matrix).pow(2)/torch.exp(z_sigma2_log_matrix),2)

        self.gaussian_pdf = self.log_pdf(self.mu_mat, self.log_sigma2_mat,self.word_embeddings_mat)
        # print (self.gaussian_pdf.size())
        self.beta = self.beta_softmax(self.gaussian_pdf)
        # exit()
        # print(self.beta.size())
        logits = torch.log(torch.mm(self.theta, self.beta)+1e-10)
        Re= - torch.sum(torch.mul(logits, doc_bow), 1)
        # print(Re)
        return Re, self.theta, self.beta #self.gaussian_pdf
#====================================================================================================================================================
class NVGMTM(nn.Module):
    def __init__(self, vocabulary_size=2000, n_hidden=256, dropout_prob=0.8, n_topics=50, topic_embeddings_size=128, inf_nonlinearity='tanh', alternative_epoch=10,gamma=0.001):
        ...
        self.gen_model=Generative_model(vocabulary_size, n_topics, topic_embeddings_size)
    def forward(self, doc_bow):
        ...
        # print(doc_bow.size(0))
        # print(self.gamma*torch.sum(torch.sum(torch.log(gaussian_pdf),1))/doc_bow.size(0))
        # print(loss)
        # print(new_loss)
        # exit()
    ...
    def train_and_eval(self, 
        ...
        for epoch in range(ckpt, training_epoch):
            ...
            if epoch+1 > 5:
                ...
                topic_coherence_file_export(mat_beta, id2word, TOPN, mat_theta, mat_log_sigma2, mat_mu, mat_word)
...
def topic_coherence_file_export(topicmat, id2word, topn, mat_theta, mat_log_sigma2, mat_mu, mat_word):
